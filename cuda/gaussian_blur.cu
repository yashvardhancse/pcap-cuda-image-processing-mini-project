...
__global__ void blur_naive_kernel(const unsigned char* input, unsigned char* output, int width, int height) {
    ...
    for (int kernel_y = -1; kernel_y <= 1; ++kernel_y) {
        for (int kernel_x = -1; kernel_x <= 1; ++kernel_x) {
            ...
            red += weight * input[sample + 0];
            green += weight * input[sample + 1];
            blue += weight * input[sample + 2];
        }
    }

    output[center + 0] = cudaimg::clamp_byte(red / 16.0f);
    output[center + 1] = cudaimg::clamp_byte(green / 16.0f);
    output[center + 2] = cudaimg::clamp_byte(blue / 16.0f);
    output[center + 3] = input[center + 3];
}

__global__ void blur_shared_kernel(const unsigned char* input, unsigned char* output, int width, int height) {
    ...
    for (int kernel_y = -1; kernel_y <= 1; ++kernel_y) {
        for (int kernel_x = -1; kernel_x <= 1; ++kernel_x) {
            ...
            const float weight = kKernelDevice[(kernel_y + 1) * 3 + (kernel_x + 1)];
            red += weight * tile[tile_index + 0];
            green += weight * tile[tile_index + 1];
            blue += weight * tile[tile_index + 2];
        }
    }

    const int output_index = cudaimg::pixel_offset(x, y, width);
    output[output_index + 0] = cudaimg::clamp_byte(red / 16.0f);
    output[output_index + 1] = cudaimg::clamp_byte(green / 16.0f);
    output[output_index + 2] = cudaimg::clamp_byte(blue / 16.0f);
    output[output_index + 3] = tile[((local_y)*tile_width + local_x) * cudaimg::kChannels + 3];
}

int run_blur_cpu_impl(const unsigned char* input_rgba, unsigned char* output_rgba, int width, int height, float strength) {
    ...
    unsigned char* first_output = output_rgba;
    unsigned char* second_output = scratch.data();
    const unsigned char* source = input_rgba;
    unsigned char* destination = first_output;

    for (int pass = 0; pass < passes; ++pass) {
        blur_once_cpu(source, destination, width, height);
        source = destination;
        destination = (destination == first_output) ? second_output : first_output;
    }
...
int run_blur_cuda_naive_impl(const unsigned char* input_rgba, unsigned char* output_rgba, int width, int height, float strength) {
    const int passes = std::max(1, static_cast<int>(std::round(strength)));
    return cudaimg::run_cuda_operation(
        ...
        [passes](cudaimg::DeviceBuffers& buffers, int launch_width, int launch_height, std::size_t bytes) {
            ...
            const auto launch_once = [&](unsigned char* source, unsigned char* destination) {
                blur_naive_kernel<<<grid, block>>>(source, destination, launch_width, launch_height);
            ...
            for (int pass = 0; pass < passes; ++pass) {
                if (launch_once(source, destination) != 0) {
                    return -1;
                }
...
int run_blur_cuda_optimized_impl(const unsigned char* input_rgba, unsigned char* output_rgba, int width, int height, float strength) {
    const int passes = std::max(1, static_cast<int>(std::round(strength)));
    return cudaimg::run_cuda_operation(
        input_rgba,
        output_rgba,
        ...
        [passes](cudaimg::DeviceBuffers& buffers, int launch_width, int launch_height, std::size_t bytes) {
            ...
            const dim3 grid = cudaimg::grid_dim(launch_width, launch_height);
            const auto launch_once = [&](unsigned char* source, unsigned char* destination) {
                blur_shared_kernel<<<grid, block>>>(source, destination, launch_width, launch_height);
                if (cudaGetLastError() != cudaSuccess) {
                    return -1;
                }
                if (cudaDeviceSynchronize() != cudaSuccess) {
...
extern "C" CUDA_IMAGE_API int gaussian_blur_cpu(const unsigned char* input_rgba, unsigned char* output_rgba, int width, int height, float strength) {
    return run_blur_cpu_impl(input_rgba, output_rgba, width, height, strength);
}

extern "C" CUDA_IMAGE_API int gaussian_blur_cuda_naive(const unsigned char* input_rgba, unsigned char* output_rgba, int width, int height, float strength) {
    return run_blur_cuda_naive_impl(input_rgba, output_rgba, width, height, strength);
}

extern "C" CUDA_IMAGE_API int gaussian_blur_cuda_optimized(const unsigned char* input_rgba, unsigned char* output_rgba, int width, int height, float strength) {
    return run_blur_cuda_optimized_impl(input_rgba, output_rgba, width, height, strength);
}
